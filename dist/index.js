(function () {
	...
	***************************************************************************** */
	...
	exports.ticker$ = _esm5.interval(1000 / 60, _esm5.animationFrameScheduler)
	    .pipe(operators.map(function () { return ({
	    deltaTime: 0,
	    time: Date.now(),
	}); }), operators.scan(function (previous, current) { return ({
	...
	var Models_1 = Models.EventType;

	var Collision = createCommonjsModule(function (module, exports) {
	Object.defineProperty(exports, "__esModule", { value: true });



	var collisionEvent$ = new _esm5.Subject();
	exports.onCollisionEvent$ = collisionEvent$.pipe(operators.map(function (collidingObjs) { return ({
	    type: Models.EventType.COLLISION,
	    data: collidingObjs,
	}); }));
	...
	function checkAndFireCollisionEvent(obj1, obj2) {
	    if (isColliding(obj1, obj2)) {
	        fireCollisionEvent(obj1, obj2);
	    }
	}
	exports.checkAndFireCollisionEvent = checkAndFireCollisionEvent;
	function fireCollisionEvent(obj1, obj2) {
	    collisionEvent$.next([obj1, obj2]);
	}
	exports.fireCollisionEvent = fireCollisionEvent;
	});

	unwrapExports(Collision);
	var Collision_1 = Collision.onCollisionEvent$;
	var Collision_2 = Collision.isColliding;
	var Collision_3 = Collision.checkForCollision;
	var Collision_4 = Collision.checkAndFireCollisionEvent;
	var Collision_5 = Collision.fireCollisionEvent;

	var Input = createCommonjsModule(function (module, exports) {
	Object.defineProperty(exports, "__esModule", { value: true });


	exports.input$ = _esm5.merge(_esm5.fromEvent(document, "keydown").pipe(operators.map(function (event) { return ({ key: event.key, pressed: true }); })), _esm5.fromEvent(document, "keyup").pipe(operators.map(function (event) { return ({ key: event.key, pressed: false }); }))).pipe(operators.distinctUntilChanged(function (prev, next) { return prev.key === next.key && prev.pressed === next.pressed; }), operators.scan(function (store, add) {
	    if (add.pressed) {
	        store.push(add.key);
	    }
	    else {
	        store = store.filter(function (key) { return add.key !== key; });
	    }
	    return store;
	}, []));
	});
	...
	var addObject$ = new _esm5.Subject();
	var removeObject$ = new _esm5.Subject();
	exports.onAddObjectEvent$ = addObject$.pipe(operators.filter(function () { return true; }), // TODO sanitize
	operators.map(function (obj) {
	    var gameObject = obj;
	    gameObject.id = uuid_1$$1.default();
	    if (!gameObject.velocity) {
	        gameObject.velocity = { x: 0, y: 0 };
	    }
	    return gameObject;
	}), operators.map(function (obj) { return ({
	    data: obj,
	    type: Models.EventType.ADD,
	}); }));
	exports.onRemoveObjectEvent$ = removeObject$.pipe(operators.filter(function (id) { return true; }), // TODO sanitize
	operators.map(function (id) { return ({
	    data: id,
	    type: Models.EventType.REMOVE,
	}); }));
	exports.objects$ = _esm5.merge(exports.onAddObjectEvent$, exports.onRemoveObjectEvent$).pipe(operators.scan(function (store, item) {
	    switch (item.type) {
	    ...
	    return store;
	}, new Map()), operators.share());
	});

	unwrapExports(Objects);
	var Objects_1 = Objects.addGameObject;
	var Objects_2 = Objects.removeGameObject;
	var Objects_3 = Objects.onAddObjectEvent$;
	var Objects_4 = Objects.onRemoveObjectEvent$;
	var Objects_5 = Objects.objects$;
	...
	var setCanvas$ = new _esm5.Subject();
	var onSetCanvasEvent$ = setCanvas$.pipe(operators.map(function (id) { return document.getElementById(id); }), operators.filter(function (canvas) { return !!canvas; }), operators.map(function (canvas) { return ({
	    canvas: canvas,
	    context: canvas.getContext("2d"),
	}); }));
	var renderer$ = _esm5.combineLatest(Core.ticker$, onSetCanvasEvent$, Objects.objects$)
	    .pipe(operators.filter(function (_a) {
	    var ticker = _a[0], _b = _a[1], canvas = _b.canvas, context = _b.context;
	    return (!!canvas && !!context);
	}), operators.sample(Core.ticker$), operators.map(function (_a) {
	...
	exports.DEFAULT_DRAW_SPRITE = DEFAULT_DRAW_SPRITE;
	function initRenderer(canvasId, drawFunction) {
	    _esm5.fromEvent(window, "load").subscribe(function () { return setCanvas$.next(canvasId); });
	...
	var Renderer_1 = Renderer.DEFAULT_DRAW_SPRITE;
	var Renderer_2 = Renderer.initRenderer;

	var dist = createCommonjsModule(function (module, exports) {
	Object.defineProperty(exports, "__esModule", { value: true });

	exports.ticker$ = Core.ticker$;

	exports.onCollisionEvent$ = Collision.onCollisionEvent$;
	exports.isColliding = Collision.isColliding;
	exports.checkForCollision = Collision.checkForCollision;
	exports.checkAndFireCollisionEvent = Collision.checkAndFireCollisionEvent;
	exports.fireCollisionEvent = Collision.fireCollisionEvent;

	exports.input$ = Input.input$;

	exports.EventType = Models.EventType;

	exports.addGameObject = Objects.addGameObject;
	exports.removeGameObject = Objects.removeGameObject;
	exports.onAddObjectEvent$ = Objects.onAddObjectEvent$;
	exports.onRemoveObjectEvent$ = Objects.onRemoveObjectEvent$;
	exports.objects$ = Objects.objects$;

	exports.DEFAULT_DRAW_SPRITE = Renderer.DEFAULT_DRAW_SPRITE;
	exports.initRenderer = Renderer.initRenderer;
	});

	unwrapExports(dist);
	var dist_1 = dist.ticker$;
	var dist_2 = dist.onCollisionEvent$;
	var dist_3 = dist.isColliding;
	var dist_4 = dist.checkForCollision;
	var dist_5 = dist.checkAndFireCollisionEvent;
	var dist_6 = dist.fireCollisionEvent;
	var dist_7 = dist.input$;
	var dist_8 = dist.EventType;
	var dist_9 = dist.addGameObject;
	var dist_10 = dist.removeGameObject;
	var dist_11 = dist.onAddObjectEvent$;
	var dist_12 = dist.onRemoveObjectEvent$;
	var dist_13 = dist.objects$;
	var dist_14 = dist.DEFAULT_DRAW_SPRITE;
	var dist_15 = dist.initRenderer;

	function bulletFactory(player, velocity) {
	    if (velocity === void 0) { velocity = { x: 0, y: -2 }; }
	    var x = (player.location.x + player.size.x / 2) - 5;
	    return ({
	        name: "bullet",
	        location: { x: x, y: player.location.y - player.size.y },
	        size: { x: 10, y: 10 },
	        velocity: velocity,
	    });
	}
	function playerFactory() {
	    return ({
	        ...
	        draw: function (gameObject, canvas, context) {
	            ...
	            context.beginPath();
	            context.rect(position.x, position.y, size.x, size.y);
	            context.rect(position.x + (size.x / 4), position.y - (size.y / 4), size.x / 2, size.y / 2);
	            context.fill();
	            context.closePath();
	        },
	    });
	}
	function enemyFactory(location) {
	    return ({
	        name: "enemy",
	        location: location,
	        size: { x: 30, y: 30 },
	...
	dist_15("game", function (_a) {
	    var canvas = _a.canvas, context = _a.context, objects = _a.objects;
	    // Drawing ui
	    // Done drawing ui
	    dist_14({ canvas: canvas, context: context, objects: objects });
	});
	combineLatest(dist_1, dist_13)
	    .pipe(sample(dist_1), map(function (_a) {
	    ...
	    dist_4.apply(void 0, gameObjects);
	});
	var inputWithObjects = combineLatest(dist_1, dist_7, dist_13)
	    .pipe(sample(dist_1), map(function (_a) {
	...
	var playerControl = inputWithObjects.pipe(map(function (_a) {
	    ...
	    var player = Array.from(objects.values())
	        .find(function (gameObject) { return gameObject.name === "player"; });
	...
	getStreamForInput("w").pipe(throttleTime(500)).subscribe(function (player) { return dist_9(bulletFactory(player)); });
	...
	getStreamForInput("s").pipe(throttleTime(1000)).subscribe(function (player) {
	    dist_9(bulletFactory(player));
	    dist_9(bulletFactory(player, { x: -2, y: -2 }));
	    dist_9(bulletFactory(player, { x: 2, y: -2 }));
	});
	dist_9(playerFactory());
	dist_9(enemyFactory({
	    x: 250,
	    y: 250,
	}));
	...
	dist_2.pipe(filter(function (_a) {
	    var data = _a.data;
	    return !!data.find(function (gameObject) { return gameObject.name === "bullet"; });
	})).subscribe(function (_a) {
	    var data = _a.data;
	    data.forEach(function (gameObject) { return dist_10(gameObject.id); });
	});
	dist_12.subscribe(function (_a) {
	    var data = _a.data;
	    return console.log("removing: ", data);
	});
	interval(5000).subscribe(function () {
	    var x = Math.floor(Math.random() * (500 - 30) - 5) + 5;
	    var y = Math.floor(Math.random() * (500 - 30) - 5) + 5;
	    dist_9(enemyFactory({ x: x, y: y }));
	});
